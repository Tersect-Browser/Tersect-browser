...
def merge_inputs(phylip_files, tsi_file=None, accessions=None):
    if accessions is None:
        return merge_phylip_files(args.phylip_files)
    else:
        all_accessions = get_accession_names(tsi_file)
        indices = [ all_accessions.index(acc) for acc in accessions ]
        indices.sort()
        return merge_phylip_files(args.phylip_files, indices=indices)
...
parser.add_argument('phylip_files', nargs='+', help='Phylip files')
parser.add_argument('-a', required=False, nargs='+', help='accessions')

args = parser.parse_args()

tmp_merged_file = merge_inputs(args.phylip_files, args.tsi_file, args.a)
